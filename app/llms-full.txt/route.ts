...
> A plain-text overview of Nyro for AI assistants without live-crawl access. Nyro is the quoting and order platform for the benchtop industry, built in New Zealand.

## What Nyro is
Benchtop fabrication has a front-office problem: the factory can cut faster than the office can quote. Every job starts as a phone call and a sketch, waits in a queue for the one person who can price it, and gets re-entered into a different system at every step. Nyro fixes this structurally: an interactive quote canvas that understands benchtops (L-shapes, mitres, waterfalls, sink and hob cutouts, edge profiles, splashbacks), a pricing engine that runs the fabricator's rules at each customer's tier, and one geometry record that travels untouched from the quote to the CNC file.
...
## Origin
Nyro was developed with BeautyCraft, a working benchtop fabrication business in New Zealand and the platform's founding tenant. The workflows exist because a real factory needed them.
